#!/usr/bin/env python3
"""Read-only, deterministic verification gate for sustained post-deployment health."""
...
class StabilityGateError(ValueError):
    """Raised when an intent or observation is malformed."""


def _string(value: Any, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise StabilityGateError(f"{field} must be a non-empty string")
    return value


def _object(value: Any, field: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise StabilityGateError(f"{field} must be an object")
    return value


def _number(value: Any, field: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise StabilityGateError(f"{field} must be a number")
    if value < 0:
        raise StabilityGateError(f"{field} must not be negative")
    return float(value)
...
def evaluate_stability(intent: dict[str, Any], observed: dict[str, Any]) -> dict[str, Any]:
    """Evaluate a post-deployment window without mutating either input."""
    ...
    if not minimum_samples.is_integer():
        raise StabilityGateError("intent.observation_window.minimum_samples must be an integer")
    ...
    if not samples.is_integer():
        raise StabilityGateError("observed.observation_window.samples must be an integer")
    samples = int(samples)
    ...
    thresholds = _object(intent.get("thresholds"), "intent.thresholds")
    max_error_rate = _number(thresholds.get("max_error_rate"), "intent.thresholds.max_error_rate")
    max_latency = _number(thresholds.get("max_p95_latency_ms"), "intent.thresholds.max_p95_latency_ms")
    max_saturation = _number(thresholds.get("max_saturation_percent"), "intent.thresholds.max_saturation_percent")
    metrics = _object(observed.get("metrics"), "observed.metrics")
    error_rate = _number(metrics.get("error_rate"), "observed.metrics.error_rate")
    latency = _number(metrics.get("p95_latency_ms"), "observed.metrics.p95_latency_ms")
    saturation = _number(metrics.get("saturation_percent"), "observed.metrics.saturation_percent")
    ...
    if not isinstance(required_checks, list) or any(not isinstance(item, str) or not item.strip() for item in required_checks):
        raise StabilityGateError("intent.required_checks must be a list of non-empty strings")
    if len(set(required_checks)) != len(required_checks):
        raise StabilityGateError("intent.required_checks must not contain duplicates")
    ...
    return {
        "allowed": not reasons,
        "state": "post_deployment_stable" if not reasons else "blocked",
        "reasons": reasons,
        ...
        "checks": {
            ...
            "metrics": not any(reason.startswith("metric_") for reason in reasons),
...
def load_json(path: Path) -> Any:
    try:
        ...
        raise StabilityGateError(f"file does not exist: {path}") from exc
    except json.JSONDecodeError as exc:
        raise StabilityGateError(f"invalid JSON: {path}") from exc


def main() -> int:
    parser = argparse.ArgumentParser(description="Verify sustained post-deployment health without changing it")
    parser.add_argument("intent_json", type=Path)
    parser.add_argument("observation_json", type=Path)
    args = parser.parse_args()
    try:
        report = evaluate_stability(load_json(args.intent_json), load_json(args.observation_json))
